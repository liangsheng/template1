...
int main()
{
    ...
    while (T--)
    {
        ...
        for (i = 0; i < n; i++)
        {
            scanf("%I64d%I64d%I64d%I64d%I64d", &b[i].p1.x, &b[i].p1.y, &b[i].p2.x, &b[i].p2.y, &zz);
            if (b[i].p1.x > b[i].p2.x) swap(b[i].p1.x, b[i].p1.x);
            if (b[i].p1.y > b[i].p2.y) swap(b[i].p1.y, b[i].p1.y);
